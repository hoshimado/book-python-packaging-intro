...
# 難読化後のPythonモジュールと実行に必要なPyArmorモジュールが格納されたフォルダへ、
# PoetryによるPKG化に必要なそれ以外のファイルをコピー
...
# * `*.whl`    PyArmorランタイムも含めたWheel形式のPKGファイル
# * `*.tar.gz` PyArmorランタイムを【追加する前】の、Poetryで出力したSdist形式のPKGファイル
